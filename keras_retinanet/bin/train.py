...
def create_models(backbone_retinanet, num_classes, weights, multi_gpu=0,
    ...
    running_mean_smooth_l1 = training_model.add_weight(dtype=tf.float32, name="running_mean_smooth_l1",
                            initializer=tf.constant_initializer(1. /9),
                            shape=[4],
                            trainable=False)                    
    running_var_smooth_l1 = training_model.add_weight(dtype=tf.float32, name="running_var_smooth_l1",
                            initializer=tf.constant_initializer(0),
                            shape=[4],
                            trainable=False)  
    ...
    # compile model
    # regression_loss = [lambda y_true,y_pred: losses.adjust_smooth_l1(y_true, y_pred, 
    #                             running_mean=running_mean_smooth_l1, running_var=running_var_smooth_l1, 
    #                             sigma_var=sigma_sq_smooth_l1)]
    training_model.compile(
        loss={
            'regression'    : regression_loss, 
            # losses.adjust_smooth_l1(
            #                     weights=[running_mean_smooth_l1, running_var_smooth_l1, sigma_sq_smooth_l1]),
...
def create_callbacks(model, training_model, prediction_model, validation_generator, sigmas, running_stats, args):
    ...
    if args.evaluation and validation_generator:
        if args.dataset_type == 'coco':
            ...
            evaluation = Evaluate(validation_generator, tensorboard=tensorboard_callback,
                                  ...
                                  sigmas=sigmas, running_stats=running_stats)
...
def main(args=None):
    ...
    if args.snapshot is not None:
        ...
        model, training_model, prediction_model, sigmas, running_stats = create_models(
